...
- **The book series** (`books/`, one generator: `books/mkbooks.js`, run from the repo
  root) is **19 volumes / 1,501 pages**. Vols 1-10 build from `SB_CONCEPTS`, 11-13 and
  15-16 from `SB_ADV_CONCEPTS`, 17-18 are collections, and two are AUTHORED from their
  own chapter files: **Vol. 14** from `books/southasia-chapters.js` and **Vol. 19
  "Named After Someone"** (the eponyms) from `books/eponym-chapters.js` — every word in
  it pulled from the app's eponym-tagged library (`t:['eponyms']`, 731 words) so no word
  data is invented. An authored volume sets `authored:true` and `src` to name which
  chapter source it indexes into. The library shelf (`books/index.html`) shows the REAL
  cover — masthead, kicker, title, tag and fact pills composed over the cover art in
  `cqw` units — not the naked illustration. Art slots live in
  `books/design-system/NANOBANANA-ALL.md` and are generated by
  `voice/pipeline/book-art-gen.py` (`NB_MODEL=gemini-3.1-flash-image` when the pro image
  model is busy); every prompt must forbid text, because the model will happily letter a
  brass plate if you name one.
