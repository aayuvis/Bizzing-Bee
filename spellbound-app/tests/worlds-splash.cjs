...
(async () => {
  ...
  const hv = await pgH.evaluate(() => { const d = document.querySelector('#sb-splash'); return {
    ...
    mates: d ? d.querySelectorAll('.spl-mate img[src$="hive-bee-fly.svg"]').length : 0,
  ...
  ok(!/hive-comb-wall\.webp/.test(idx0) && !/hive-cell\.webp/.test(idx0),
    'the photo comb plates stay cut — the wall and the cell are drawn SVG (the bees are the painted sprites)');
  ok(/splcap/.test(idx0) && /splvin/.test(idx0) && /splblur/.test(idx0),
    'the refined comb: capped-wax cells, a vignette, and the door\'s blurred halo');
