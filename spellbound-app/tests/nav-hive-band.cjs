  ...
  for (const vp of [{width:1180,height:1000,n:'desktop'},{width:390,height:844,n:'phone'}]) {
    ...
    const r = await pg.evaluate(() => {
      ...
      if (band && rings) { const br = band.getBoundingClientRect(), rr = rings.getBoundingClientRect();
        ...
        // the level must live INSIDE the daily-goal card, not beside it
        const card = rings.closest('.sb-card');
        out.sameCard = !!(card && card.contains(band));
        out.bandOnHome = true;
    ...
    if (r.sameCard === false) errs.push(vp.n + ': the spelling level is outside the daily-goal card');
