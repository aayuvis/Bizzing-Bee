# EXTEND THE MOMENT — The Receipt
### Second format: the price stack, with the ending inverted

Everyone over forty knows the cadence: a stack of itemised prices, then a final line that money can't buy.
**Bromic's version inverts the ending** — because the whole argument is that the thing which unlocks the
backyard *is* buyable, and it is the cheapest line on the list.

That inversion is what makes it a Bromic joke rather than a borrowed one. It is also the most on-strategy
idea in the platform: it states the argument as arithmetic, in public, in the customer's own numbers.

## Why it earns its place

The films carry the **fame**; the receipts carry the **argument**. That is a healthier platform than films
alone, and it fixes the one real weakness of the before/after device — every execution needs a shoot.
A receipt is pure typography: near-zero cost, produced in a day, endlessly renewable, and it re-skins
instantly for a homeowner, a builder or an operator by swapping the line items. It is also the most
screenshot-able asset in the platform and the easiest thing a dealer can put on a counter card.

> **Craft note.** Set these as real typography, never as AI-generated images — image models render text
> unreliably, and this format is nothing but text and numbers.

---

## Executions

### The Backyard
*Consumer · homeowner*

| | |
|---|---:|
| Outdoor sofa and dining set | $11,400 |
| Pergola | $24,000 |
| Outdoor kitchen | $41,000 |
| Wood-fired oven | $9,000 |
| Landscaping and lighting | $16,000 |
| **Total** | **$101,400** |
| **Being able to sit in any of it in October** | **$4,200** |

**Extend the Moment.**

---

### The Rough-In
*Trade · custom builder*

| | |
|---|---:|
| Cedar-lined ceiling | $18,000 |
| Stone paving | $31,000 |
| Integrated lighting | $7,200 |
| Cutting all of it open later to run a circuit | $6,000 |
| **What that change order costs everyone** | **$6,000** |
| **Roughing it in at framing, before any of it exists** | **$400** |

**Extend the Moment.**

---

### The Covers
*Commercial · hospitality*

| | |
|---|---:|
| Terrace fit-out | $54,000 |
| Furniture, planters and screens | $46,000 |
| Forty covers, sold out every night in February | $312,000 |
| The same forty covers, every night in May | $0 |
| **Revenue the terrace cannot earn in May** | **$104,000** |
| **Making May sellable** | **$9,000** |

**Extend the Moment.**

---

### Buying It Twice
*Commercial · procurement*

| | |
|---|---:|
| Patio heater | $89 |
| Patio heater, again | $89 |
| Patio heater, again | $89 |
| …and eleven more over ten years | $979 |
| Gas, ten years | $4,100 |
| Staff hours spent moving bottles | $18,000 |
| **Ten years of not solving it** | **$23,346** |
| **Solving it once** | **$9,000** |

**Extend the Moment.**

---

---

## The visual campaign — The Priced Scene

The receipt told in type; this tells it in the space itself. One photograph of a real build,
every object wearing its price like a showroom tag, the total climbing as the tags land — then
the last tag drops on the only thing nobody costed, and it is the cheapest number on screen.

**The backyard.** $16,000 planting and lighting · $11,400 sectional · $24,000 pergola ·
$41,000 outdoor kitchen · $9,000 wood-fired oven · $19,000 stone paving.
**Running total: $120,400.**
Then, on the heater: **being able to use any of it in October — $4,200.**

**The terrace.** $14,000 planters and screens · $46,000 tables, chairs and glassware ·
$8,000 festoon and downlighting · $54,000 service station and fit-out · and forty covers roped
off till spring, at $0. Then: **making those forty covers sellable in May — $9,000.**

### Why this is the most efficient asset in the platform

One photograph carries the whole argument. No cast, no dialogue, no shoot day beyond the plate
itself — and it re-skins for any audience by swapping the labels. It is the natural hero for
print, out-of-home, a showroom wall and a dealer counter card, and it crops from a six-sheet to
a 9:16 story without losing the joke.

It also unifies the platform: the priced scene takes the same cold-to-warm turn as the films,
so both formats resolve the same way.

### Craft mandate

Set the tags as **live typography over the photograph** — never baked into an AI-generated
image, and never as flattened artwork. Keeping the tags as a separate layer means the same
plate re-prices for a different market, currency or spec in minutes, and the numbers stay
legible at every crop. Tags near the frame edge anchor inward so nothing clips.

All figures are illustrative and must be replaced with verified pricing before anything runs.

---

## Legal — read before this goes anywhere

**Never use the word that ends the original.** It is a registered Mastercard trademark and among the most
aggressively protected properties in advertising.

Do not reproduce their sign-off construction, their voiceover cadence, their type treatment, or their card.
The itemised price stack itself is a generic device used widely and is not theirs — the protected assets are
the wording and the lockup.

The inversion helps here as well as creatively: we are not claiming anything is unbuyable, we are claiming
the opposite. Still, have counsel clear the final wording before production. This is the one idea in the
deck where the legal review is not a formality.

All figures above are illustrative and must be replaced with verified pricing before anything runs.
